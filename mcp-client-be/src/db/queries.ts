...
export async function updateNetwork(modelNetwork: string) {
  const existing = await getUserSettings();

  if (existing.length === 0) {
    return db
      .insert(userSettings)
      .values({ modelNetwork, modelName: "" })
      .returning();
  }
  return db
    .update(userSettings)
...
export async function updateModel({
  ...
  return db
    ...
    .where(
      and(
        eq(userSettings.modelNetwork, modelNetwork),
        eq(userSettings.id, existing[0].id),
      ),
    );
}
...
export async function updateMcpServer(
  id: number,
  data: { name?: string; command?: string; args?: string[]; cwd?: string },
) {
  return db
    .update(mcpServers)
    .set(data)
...
// Add a tool
export async function addMcpTool(data: {
  mcpServerId: number;
  name: string;
  enabled?: boolean;
}) {
